...
exports.TriviaApiClient = exports.BUNDLED_JARCHIVE_CLUES = void 0;
 ...
 */
const CATEGORY_DISTRACTOR_POOLS = {
    science: [
 ...
 */
exports.BUNDLED_JARCHIVE_CLUES = [
    // --- SCIENCE ($200 to $2000) ---
    {
        round: 'Jeopardy!',
...
class TriviaApiClient {
     ...
     */
    static mapJArchiveValueToElo(round, valueRaw) {
        ...
        const isDoubleJeopardy = round.toLowerCase().includes('double') || numericValue >= 1200;
     ...
     */
    static classifyCategory(rawCat) {
     ...
     */
    static generateFourOptions(correctAnswer, category, providedDistractors) {
        const distractors = [];
     ...
     */
    static convertJArchiveToQuestion(clue, index = 0) {
        ...
        const options = this.generateFourOptions(clue.answer, category, clue.distractors);
        const wikiUrl = (0, wikipedia_1.formatWikipediaUrl)(clue.answer, clue.wikipedia_url);
        return {
    ...
    static async fetchQuestions(params) {
                ...
                if (Array.isArray(json) && json.length > 0) {
                    return json.map((item, idx) => {
                        if (item.clue_text && item.answer && item.options) {
                            return {
                                ...item,
                                wikipedia_url: (0, wikipedia_1.formatWikipediaUrl)(item.answer, item.wikipedia_url),
                            };
                        }
                        return this.convertJArchiveToQuestion(item, idx);
                    });
                }
            }
        }
        ...
        let pool = exports.BUNDLED_JARCHIVE_CLUES.map((clue, idx) => this.convertJArchiveToQuestion(clue, idx));
        if (category !== 'all') {
            pool = pool.filter((q) => q.category === category);
        }
        // Sort closest to target Elo
        pool.sort((a, b) => Math.abs(a.elo_rating - targetElo) - Math.abs(b.elo_rating - targetElo));
        return pool.slice(0, count);
    }
}
exports.TriviaApiClient = TriviaApiClient;
